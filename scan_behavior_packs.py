#!/usr/bin/env python3
"""
Behavior Pack Manifest Scanner

Scans the behavior_packs directory for manifest.json files,
extracts UUID and version from each, and outputs to world_behavior_packs.json.
...
def scan_behavior_packs():
    # Get the directory where this script is located
    script_dir = Path(__file__).parent.resolve()
    
    behavior_packs_dir = script_dir / "behavior_packs"
    output_file = script_dir / "world_behavior_packs.json"
    
    if not behavior_packs_dir.exists():
        print(f"Error: behavior_packs directory not found at {behavior_packs_dir}")
        return
    ...
    print(f"Scanning behavior packs in: {behavior_packs_dir}\n")
    
    # Iterate through all subdirectories in behavior_packs
    for folder in sorted(behavior_packs_dir.iterdir()):

#!/usr/bin/env python3
"""
Minecraft Bedrock .mcaddon Extractor

Scans the downloaded_addons directory for .mcaddon files, extracts them,
...
def main():
    ...
    print("=" * 60)
    print("  Minecraft Bedrock .mcaddon Extractor")
    print("=" * 60)
    ...
    if not downloaded_addons_dir.exists():
        ...
        print("Please create the folder and place .mcaddon files inside.")
        input("\nPress Enter to exit...")
        return
    
    # Find all .mcaddon files
    mcaddon_files = list(downloaded_addons_dir.glob("*.mcaddon"))
    
    if not mcaddon_files:
        print(f"\nNo .mcaddon files found in {downloaded_addons_dir}")
        input("\nPress Enter to exit...")
        return
